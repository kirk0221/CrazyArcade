...
public class Keppy extends Character implements KeyListener{
	...
	public Keppy(Screen screen, int playertype) { /*플레이어 타입을 전달받아, 해당 타입에 따라 키에 대한 동작이 다르도록 함*/
		...
		Image keppy_front = new ImageIcon("Resources/kephi_front.png").getImage();//케피 정면 이미지
		Image keppy_back = new ImageIcon("Resources/kephi_back.png").getImage();//케피 후면 이미지
		Image keppy_left = new ImageIcon("Resources/kephi_left.png").getImage();//케피 좌측면 이미지
		Image keppy_right = new ImageIcon("Resources/kephi_right.png").getImage();//케피 우측면 이미지
